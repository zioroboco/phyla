import { ActorRef, createMachine, createSchema, send } from "xstate"
...
export type Watcher = ActorRef<WatcherEvent>
  ...
  },
  {
    actions: {
      startWatchers: context => {
        context.watchers.forEach(watcher => send({ type: "START" }, watcher))
      },
      stopWatchers: context => {
        context.watchers.forEach(watcher => send({ type: "STOP" }, watcher))
      },
    },
  }
)
